...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const translations = {
    en: {
      ...
      privacy_notice: "We use Cloudflare Web Analytics to measure aggregate visits, including pages, referrers, approximate country, device type, and performance. It does not use cookies, analytics localStorage, or individual fingerprinting. Theme, language, and interface preferences are saved only in your browser.",
    ...
    ru: {
      ...
      privacy_notice: "Мы используем Cloudflare Web Analytics для агрегированной статистики посещений: страницы, источники переходов, примерная страна, тип устройства и скорость загрузки. Сервис не использует cookies, localStorage для аналитики или цифровые отпечатки посетителей. Настройки темы, языка и интерфейса сохраняются только в вашем браузере.",
    ...
    zh: {
      ...
      privacy_summary: "隐私",
      privacy_notice: "我们使用 Cloudflare Web Analytics 统计汇总访问数据，包括页面、来源网站、大致国家或地区、设备类型和性能。该服务不使用 Cookie、用于分析的 localStorage，也不会对访客进行个人指纹识别。主题、语言和界面偏好仅保存在您的浏览器中。",
      placeholder_name: "姓名",
      placeholder_email: "hello@company.com",
    ...
    ja: {
      ...
      privacy_notice: "当サイトでは Cloudflare Web Analytics を使用し、閲覧ページ、参照元、おおよその国、デバイスの種類、パフォーマンスなどの訪問状況を集計しています。Cookie、解析用 localStorage、個人を識別するフィンガープリントは使用しません。テーマ、言語、インターフェースの設定はブラウザ内にのみ保存されます。",
